...
MOSTRO_SRC := $(shell find ./mostro/src) mostro/Cargo.toml mostro/Cargo.lock
...
rebranding:
	...
	find . \( -name "*.md" -o -name ".gitignore" -o -name "manifest.yaml" -o -name "*Service.yml" \) -type f -not -path "./mostro/*" -exec sed -i '' -e "s/mostro/$$NEW_PKG_ID/g; s/Hello World/$$NEW_PKG_TITLE/g" {} +; \
...
docker-images/aarch64.tar: Dockerfile docker_entrypoint.sh mostro/target/aarch64-unknown-linux-musl/release/mostrod
ifeq ($(ARCH),x86_64)
else
...
docker-images/x86_64.tar: Dockerfile docker_entrypoint.sh mostro/target/x86_64-unknown-linux-musl/release/mostrod
ifeq ($(ARCH),aarch64)
else
	mkdir -p docker-images
...
mostro/target/aarch64-unknown-linux-musl/release/mostro: $(MOSTRO_SRC)
	docker run --rm -v ~/.cargo/registry:/root/.cargo/registry -v "$(shell pwd)"/mostro:/home/rust/src messense/rust-musl-cross:aarch64-musl cargo build --release

mostro/target/x86_64-unknown-linux-musl/release/mostro: $(MOSTRO_SRC)
	docker run --rm -v ~/.cargo/registry:/root/.cargo/registry -v "$(shell pwd)"/mostro:/home/rust/src messense/rust-musl-cross:x86_64-musl cargo build --release
